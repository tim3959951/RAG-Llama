...
def get_time_tool2(query: str) -> datetime:
    ...
    ("上週一早上九點", (now - timedelta(days=(now.weekday() + 7))).replace(hour=9, minute=0, second=0)),
    ("下週一", (now + timedelta(days=(7 - now.weekday()))).replace(hour=12, minute=0, second=0)),
    ("上週五", (now - timedelta(days=(now.weekday() - 4 + 7) % 7)).replace(hour=12, minute=0, second=0)),
    ("下週五", (now + timedelta(days=(4 - now.weekday() + 7) % 7)).replace(hour=12, minute=0, second=0)),
