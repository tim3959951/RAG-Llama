...
def get_time_tool2(query: str) -> datetime:
    ...
    ("五個小時後", now + timedelta(hours=5)),
    ("等一下", now + timedelta(hours=2)),
    ("很快", now + timedelta(minutes=30)),
    ("過一下子", now + timedelta(minutes=15)),
    ("待會", now + timedelta(hours=1)),
    ("明天下午三點", now.replace(hour=15, minute=0, second=0) + timedelta(days=1)),
    ("後天上午十點", now.replace(hour=10, minute=0, second=0) + timedelta(days=2)),
    ("前天", (now - timedelta(days=2)).replace(hour=12, minute=0, second=0)),
    ("前天", (now - timedelta(days=3)).replace(hour=12, minute=0, second=0)),
    ("上個禮拜三", (now - timedelta(days=(now.weekday() - 2 + 7) % 7 + 7)).replace(hour=12, minute=0, second=0)),
    ("上個星期四", (now - timedelta(days=(now.weekday() - 3 + 7) % 7 + 7)).replace(hour=12, minute=0, second=0)),
    ("上週五", (now - timedelta(days=(now.weekday() - 4 + 7) % 7 + 7)).replace(hour=12, minute=0, second=0)),
    ("上週六", (now - timedelta(days=(now.weekday() - 5 + 7) % 7 + 7)).replace(hour=12, minute=0, second=0)),
    ("上週日", (now - timedelta(days=(now.weekday() - 6 + 7) % 7 + 7)).replace(hour=12, minute=0, second=0)),
    ("上禮拜天", (now - timedelta(days=(now.weekday() - 6 + 7) % 7 + 7)).replace(hour=12, minute=0, second=0)),
    ("下週一", (now + timedelta(days=(7 - now.weekday()))).replace(hour=12, minute=0, second=0)),
    ("十個小時後", now + timedelta(hours=10)),
    ("這個週末", (now + timedelta(days=(5 - now.weekday()) % 7)).replace(hour=10, minute=0, second=0)),
    ("下個週末", (now + timedelta(days=((5 - now.weekday()) % 7) + 7)).replace(hour=10, minute=0, second=0)),
    ("明天一大早", now.replace(hour=6, minute=0, second=0) + timedelta(days=1)),
    ("兩個小時後", now + timedelta(hours=2)),
    ("一個小時後", now + timedelta(hours=1)),
    ("三十分鐘後", now + timedelta(minutes=30)),
    ("幾分鐘後", now + timedelta(minutes=10)),
    ("上個月", (now - timedelta(days=30)).replace(hour=12, minute=0, second=0)),
    ("昨天", (now - timedelta(days=1)).replace(hour=12, minute=0, second=0)),
